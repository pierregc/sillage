...
final class SimulationModel: ObservableObject {
    ...
    // Read and written by the camera extension next door. Swift's `private` is file-scoped,
    // so these two cannot stay private; neither is published, so no view can see them.
    var lastDrawTime: CFTimeInterval?
    var manualCameraUntil: Date?
